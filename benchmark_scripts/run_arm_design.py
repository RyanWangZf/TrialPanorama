#!/usr/bin/env python3
"""
Runner script for the clinical trial arm design QA benchmark.
"""
import os
import json
import argparse
...
def run_benchmark(args):
    """
    Run the arm design QA benchmark with the given arguments.
    ...
    # Set task name
    task_name = "arm_design"
    
    # Load dataset
    train_dataset, dataset = data.load_qa_task_data(task_name, task_config.data_path)
    
    if not dataset:
        raise ValueError(f"No data found for arm design task")
    
    print(f"Loaded {len(train_dataset)} train samples and {len(dataset)} test samples for arm design task")
    ...
    # Run benchmark
    print(f"Running arm design benchmark with model {model_config.model_name}...")
...
def main():
    ...
    parser = config.create_arg_parser("arm_design")
