#!/usr/bin/env python3
"""
Runner script for the clinical trial sample size estimation QA benchmark.
"""
...
def run_benchmark(args):
    """
    Run the sample size estimation QA benchmark with the given arguments.
    ...
    # Set task name
    task_name = "sample_size_estimation"
    
    # Load dataset
    train_dataset, dataset = data.load_qa_task_data(task_name, task_config.data_path)
    
    if not dataset:
        raise ValueError(f"No data found for sample size estimation task")
    
    print(f"Loaded {len(train_dataset)} train samples and {len(dataset)} test samples for sample size estimation task")
    ...
    # Run benchmark
    print(f"Running sample size estimation benchmark with model {model_config.model_name}...")
...
def main():
    ...
    parser = config.create_arg_parser("sample_size_estimation")
