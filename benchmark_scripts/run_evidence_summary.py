#!/usr/bin/env python3
"""
Runner script for the clinical trial evidence summary QA benchmark.
"""
...
def run_benchmark(args):
    """
    Run the evidence summary QA benchmark with the given arguments.
    ...
    if not dataset:
        raise ValueError(f"No data found for evidence summary task")
    
    print(f"Loaded {len(train_dataset)} train samples and {len(dataset)} test samples for evidence summary task")
    ...
    # Run benchmark
    print(f"Running evidence summary benchmark with model {model_config.model_name}...")
...
def main():
    ...
    parser = config.create_arg_parser("evidence_summary")
