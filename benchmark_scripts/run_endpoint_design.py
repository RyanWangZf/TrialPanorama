#!/usr/bin/env python3
"""
Runner script for the clinical trial endpoint design QA benchmark.
"""
...
def run_benchmark(args):
    """
    Run the endpoint design QA benchmark with the given arguments.
    ...
    # Set task name
    task_name = "endpoint_design"
    
    # Load dataset
    train_dataset, dataset = data.load_qa_task_data(task_name, task_config.data_path)
    
    if not dataset:
        raise ValueError(f"No data found for endpoint design task")
    
    print(f"Loaded {len(train_dataset)} train samples and {len(dataset)} test  samples for endpoint design task")
    ...
    # Run benchmark
    print(f"Running endpoint design benchmark with model {model_config.model_name}...")
...
def main():
    ...
    parser = config.create_arg_parser("endpoint_design")
