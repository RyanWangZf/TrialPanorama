...
BENCHMARK_DATA_PATH = os.path.join(os.getenv("DATABASE_PATH"), "benchmark_data/final_results/study_search")
DEFAULT_OUTPUT_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "benchmark_results")

def study_search_data_eda():
    """
    Perform exploratory data analysis on the study search data.
    """
    ...
    for test_data in stats["test"]:
        num_test_included += test_data.get('included_pmids_count', 0)
    print("Total number of test studies included:", num_test_included)

    num_train_included = 0
    for train_data in stats["train"]:
        num_train_included += train_data.get('included_pmids_count', 0)
    print("Total number of train studies included:", num_train_included)
    pass

def load_study_search_results():
    """Load the study search results."""
    all_models = ["gpt-4o-mini", "gpt-4o", "o3-mini"]
    all_results = []
    for model in all_models:
        result_folders = glob(os.path.join(DEFAULT_OUTPUT_PATH, "study_search", f"{model}/*"))
    ...
    all_results.to_csv(os.path.join(DEFAULT_OUTPUT_PATH, "study_search", "all_results.csv"), index=False)

if __name__ == "__main__":
    # load the results
    study_search_data_eda()
    load_study_search_results()
